...
async function initializeCrisisMode() {
  ...
  if (cachedRequests.length === 0) {
    console.log('[SW] Initializing crisis mode - caching resources');
    ...
    event.respondWith(
      (async () => {
        try {
          ...
          console.log(`[SW] Crisis resource loaded in ${endTime - startTime}ms: ${event.request.url}`);
          ...
          console.log(`[SW] Crisis resource served from cache in ${endTime - startTime}ms: ${event.request.url}`);
...
console.log(`[SW] Astral Core Service Worker ${SW_VERSION} initialized`);
console.log(`[SW] Crisis mode: ${CRISIS_MODE_ENABLED ? 'ENABLED' : 'DISABLED'}`);
