...
const envSchema = z.object({
  ...
  JWT_ALGORITHM: z.enum(['HS256', 'HS384', 'HS512', 'RS256', 'RS384', 'RS512']).optional(),
  ...
  VITE_PWA_THEME_COLOR: z.string().regex(/^#[0-9A-Fa-f]{6}$/).default('#667eea'),
  VITE_PWA_BACKGROUND_COLOR: z.string().regex(/^#[0-9A-Fa-f]{6}$/).default('#ffffff'),
  VITE_PWA_DISPLAY: z.enum(['fullscreen', 'standalone', 'minimal-ui', 'browser']).default('standalone'),
  VITE_PWA_ORIENTATION: z.enum(['any', 'natural', 'landscape', 'portrait']).default('portrait'),
...
export function validateEnvironment(env: NodeJS.ProcessEnv = process.env): ValidationResult {
  ...
  const publicVars = Object.keys(env).filter(key => key.startsWith('VITE_'));
