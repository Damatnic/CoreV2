...
const loadTranslations = async (language: string, namespace: string) => {
  try {
    ...
    console.warn(`[i18n] Failed to load translations for ${language}:${namespace}`, error);
    return {};
  }
};
...
i18n
  ...
  .init({
    ...
    returnObjects: false, // Disable object returns to prevent React rendering issues
...
export const getCulturalContext = (language: string): CulturalContext => {
...
export const getMentalHealthStigmaLevel = (language: string): StigmaLevel => {
  return getCulturalContext(language).mentalHealthStigma;
};

export const getCrisisEscalationPreference = (language: string): EscalationPreference => {
  return getCulturalContext(language).crisisEscalationPreference;
};

export const getPreferredCommunicationStyle = (language: string): CommunicationStyle => {
...
export const getCrisisTranslation = (key: string, options?: any): string => {
  const result = i18n.t(`crisis:${key}`, options);
  return typeof result === 'string' ? result : String(result);
};

export const getCommonTranslation = (key: string, options?: any): string => {
